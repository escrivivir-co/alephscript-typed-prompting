...
export default function SchemaCreator() {
    ...
    const editIdNumber = editId ? parseInt(editId, 10) : undefined;
    const prefilledTypeScript = searchParams.get("typescript");

    const [typeScript, setTypeScript] = useState(prefilledTypeScript || "");
    ...
    const { data: editSchema, isLoading: isLoadingSchema } = useQuery<Schema>({
        queryKey: [`/api/schemas/${editIdNumber}`],
        enabled: !!editIdNumber && !isNaN(editIdNumber),
    });
    ...
    const createSchemaMutation = useMutation({
        mutationFn: async (jsonSchema: string) => {
            const endpoint = editIdNumber
                ? `/api/schemas/${editIdNumber}`
                : "/api/schemas";

            const method = editIdNumber ? "PUT" : "POST";
    ...
    const handleConvert = async () => {
        try {
			...
			if (!typeScript.includes("TypedMessage")) {
				setTypeScript(missingDefault(typeScript));
				return;
			}
            const jsonSchema =
                convertTypeScriptToJsonSchema(combinedTypeScript);
	...
	function missingDefault(code: string) {
		if (!code.includes("TypedMessage")) {
			toast({
				title: "Warning",
				description: "TypedMessage not found. Appending TypedMessage interface automatically.",
				variant: "destructive",
			});
			code += "\nexport interface TypedMessage { content: any }";
		}
		return code;
	}
    ...
    return (
        <div className="container mx-auto p-6">
            <Card>
                <CardHeader>
                    <CardTitle>
                        {editIdNumber ? "Edit Schema" : "Create New Schema"}
                    </CardTitle>
                </CardHeader>
                                        ...
                                        <div className="space-y-4">
                                            {schemas?.map((schema) => (
                                                ...
                                                >
                                                    <Checkbox
                                                        id={`schema-${schema.id}`}
                                                        checked={selectedSchemas.includes(
                                                            schema.id
                                                        )}
                                                        onCheckedChange={() =>
                                                            handleSchemaSelect(
